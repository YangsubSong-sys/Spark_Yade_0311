...
void CMSFlexGrid::SetCurCell(int row, int col)
{
	SetRow(row);
	SetCol(col);
}

void CMSFlexGrid::GetCurCell(int& row, int& col)
{
	row = GetRow();
	col = GetCol();
}

void CMSFlexGrid::SetRowColor(int row, COLORREF ForeColor, COLORREF BackColor)
{
	SetRow(row);
	SetRowColor(ForeColor, BackColor);
}

void CMSFlexGrid::SetRowColor(COLORREF ForeColor, COLORREF BackColor)
{
	int cols = GetCols();
	for (int col=1; col<cols; col++) {
		SetCol(col);
		SetCellForeColor(ForeColor);
		SetCellBackColor(BackColor);
	}
}


void CMSFlexGrid::SetColColor(int col, COLORREF ForeColor, COLORREF BackColor)//sylee
{
	int rows = GetRows();
